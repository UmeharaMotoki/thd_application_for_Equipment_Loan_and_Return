...
import { newAdditionalUserRow } from "@/lib/equipment-lending-form/lendingFormUtils";
...
export default function EquipmentLendingApplication() {
  ...
  const userMode = useMemo(
    () => deriveLendingUserMode(additionalUsers),
    [additionalUsers],
  );
  ...
  }, [applicantData, userData, deliveryData, lendingLines, reasonData, additionalUsers, userReasonByEmp]);
  ...
  const handleUserNext = (event: FormEvent<HTMLFormElement>) => {
    ...
    for (const row of additionalUsers) {
      if (!row.userEmployeeNumber.trim()) {
        setMessage({
          type: "error",
          text: "追加利用者の社員番号をすべて入力してください。未使用の行は削除してください。",
        });
        return;
      }
    }
        ...
        <Alert severity="warning" sx={{ mb: 2 }}>
          ...
          {step === "notice" ? (
            <Stack spacing="20px">
              ...
              <NoticeBulletList items={IT_SERVICE_WARNINGS} />
              <Box
                sx={{
                  pt: "15px",
                  mt: "40px",
            ...
            <>
              ...
              <Box component="form" onSubmit={handleUserNext} sx={{ width: "90%", margin: "0 auto" }}>
                <Stack spacing={2.2}>
                  <Typography sx={{ fontWeight: 700, fontSize: 16, color: "#007D9E", mb: 0.5 }}>
                    代表利用者
                  </Typography>
                  <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
